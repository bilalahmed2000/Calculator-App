...
export default function BoatLoanCalculator() {
  ...
  function calculate() {
    ...
    const price = parseN(boatPrice), dp = parseN(downPayment), ti = parseN(tradeIn), tax = parseN(salesTax), r = parseN(rate), ty = parseN(termY);
    if (!(price > 0)) { setErr("Boat price must be greater than 0."); return; }
    if (!(r >= 0)) { setErr("Interest rate must be 0 or greater."); return; }
    if (!(ty > 0)) { setErr("Loan term must be greater than 0."); return; }
    const taxAmt = (price - ti) * tax / 100;
    const P = price + taxAmt - dp - ti;
    if (P <= 0) { setErr("Down payment and trade-in exceed the boat price."); return; }
    ...
    setResult({ pi, price, dp, taxAmt, tradeIn: ti, loanAmount: P, totalInt, totalPaid: P + totalInt, upfront: dp + taxAmt, rows, annual, payoff: rows[rows.length - 1]?.label });
  }

  function clear() { setBoatPrice("35000"); setDownPayment("7000"); setTradeIn("0"); setSalesTax("6"); setRate("7.5"); setTermY("10"); setResult(null); setErr(""); }

  return (
    <div className="calc-wrap">
      <header className="calc-hero">
        <h1>Boat Loan Calculator</h1>
        <p className="muted">Calculate your boat loan monthly payments including sales tax and trade-in value. See full amortization schedule.</p>
      </header>
      <div style={{ maxWidth: 1120, margin: "0 auto" }}>
        <div style={{ display: "flex", gap: 20, alignItems: "flex-start", flexWrap: "wrap", marginBottom: 20 }}>
          <section className="card" style={{ flex: "0 0 312px", minWidth: 268 }}>
            <div style={fst}><label style={lst}>Boat Price</label>{row([<span style={sym}>$</span>, <input style={ist} value={boatPrice} onChange={e => setBoatPrice(e.target.value)} />])}</div>
            ...
            <div style={fst}><label style={lst}>Trade-in Value</label>{row([<span style={sym}>$</span>, <input style={ist} value={tradeIn} onChange={e => setTradeIn(e.target.value)} />])}</div>
          ...
          {result && (
            <section className="card" style={{ flex: 1, minWidth: 300 }}>
              <div style={{ display: "flex", gap: 24, flexWrap: "wrap", alignItems: "center", marginBottom: 20 }}>
                <DonutChart principal={result.loanAmount} interest={result.totalInt} />
                <div style={{ flex: 1, minWidth: 200 }}>
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "10px 20px" }}>
                    {[["Monthly Payment", ccy(result.pi)], ["Loan Amount", ccy(result.loanAmount)], ["Sales Tax", ccy(result.taxAmt)], ["Total Interest", ccy(result.totalInt)], ["Total Cost", ccy(result.totalPaid)], ["Payoff Date", result.payoff]].map(([l, v]) => (
