...
export default function PersonalLoanCalculator() {
  ...
  const [termY, setTermY] = useState("3");
  ...
  function calculate() {
    ...
    const P = parseN(amount), r = parseN(rate), ty = parseN(termY), tm = parseN(termM), ex = parseN(extra);
    ...
    let bal = P, totalInt = 0, rows = [], startD = new Date(TODAY.getFullYear(), TODAY.getMonth() + 1, 1);
    for (let i = 0; i < n && bal > 0.005; i++) {
      ...
      rows.push({ n: i + 1, label: MONTHS[d.getMonth()] + " " + d.getFullYear(), interest: intP, principal: prinP, extra: exP, balance: Math.max(bal, 0), cumInt: totalInt });
      if (bal <= 0.005) break;
    }
    const annual = [];
    ...
    setResult({ pi, monthly: pi + ex, rows, annual, principal: P, totalInt, totalPaid: P + totalInt, payoff: lastRow?.label });
  }

  function clear() { setAmount("10000"); setRate("7.5"); setTermY("3"); setTermM("0"); setExtra("0"); setResult(null); setErr(""); }

  return (
    <div className="calc-wrap">
      <header className="calc-hero">
        <h1>Personal Loan Calculator</h1>
        <p className="muted">Calculate monthly payments, total interest, and see a full amortization schedule for your personal loan.</p>
      </header>
      <div style={{ maxWidth: 1120, margin: "0 auto" }}>
        <div style={{ display: "flex", gap: 20, alignItems: "flex-start", flexWrap: "wrap", marginBottom: 20 }}>
          <section className="card" style={{ flex: "0 0 312px", minWidth: 268 }}>
            ...
            <div style={fst}><label style={lst}>Annual Interest Rate (APR)</label>{row([<input style={{ ...ist, width: "100%" }} value={rate} onChange={e => setRate(e.target.value)} />, <span style={sym}>%</span>])}</div>
            ...
            <section className="card" style={{ flex: 1, minWidth: 300 }}>
              <div style={{ display: "flex", gap: 24, flexWrap: "wrap", alignItems: "center", marginBottom: 20 }}>
                ...
                <div style={{ flex: 1, minWidth: 200 }}>
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "10px 20px" }}>
                    {[["Monthly Payment", ccy(result.pi)], ["Loan Amount", ccy(result.principal)], ["Total Interest", ccy(result.totalInt)], ["Total Cost", ccy(result.totalPaid)], ["Payoff Date", result.payoff]].map(([l, v]) => (
              ...
              <div style={{ overflowX: "auto" }}>
                <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
                  <thead><tr style={{ background: "#f0f0ff" }}>{["Date","Principal","Interest","Extra","Balance"].map(h => <th key={h} style={{ padding: "8px 10px", textAlign: "right", fontWeight: 700, color: "#4f46e5", whiteSpace: "nowrap" }}>{h}</th>)}</tr></thead>
                  <tbody>
